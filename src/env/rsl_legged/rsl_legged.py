...
from agents.roa_ppo import LeggedRobotCfg as LeggedRobotCfg
from agents.roa_ppo import LeggedRobotCfgPPO as LeggedRobotCfgPPO
...
# torch must be imported AFTER isaacgym
...
from env.rsl_legged.utils.terrain import Terrain
from env.rsl_legged.utils.math import *
from env.rsl_legged.utils.helpers import class_to_dict
...
# for task registry
from env.rsl_legged.utils.helpers import get_args, update_cfg_from_args, class_to_dict
from env.rsl_legged.utils.helpers import set_seed, parse_sim_params
from datetime import datetime

# global variable
PATH_TO_ROBOT_URDF = "/rsl_legged/resources/"
...
class A1ParkourCfg( LeggedRobotCfg ):
    ...
    class asset( LeggedRobotCfg.asset ):
        ...
        file = '{LEGGED_GYM_ROOT_DIR}/' + PATH_TO_ROBOT_URDF + '/a1/urdf/a1.urdf'
        foot_name = "foot"
...
class LeggedRobot(BaseTask):
    ...
    if self.cfg.domain_rand.action_delay:
      if self.global_counter % self.cfg.domain_rand.delay_update_global_steps == 0:
        if len(self.cfg.domain_rand.action_curr_step) != 0:
          self.delay = torch.tensor(self.cfg.domain_rand.action_curr_step.pop(
              0), device=self.device, dtype=torch.float)
      if self.viewer:
  ...
  def _update_goals(self):
    ...
    self.reached_goal_ids = torch.norm(
        self.root_states[:, :2] - self.cur_goals[:, :2], dim=1) < self.cfg.env.next_goal_threshold
  ...
  def post_physics_step(self):
    ...
    if self.viewer and self.enable_viewer_sync and self.debug_viz:
      ...
      if self.cfg.depth.use_camera:
        ...
        cv2.imshow(
            "Depth Image", self.depth_buffer[self.lookat_id, -1].cpu().numpy() + 0.5)
        cv2.waitKey(1)
  ...
  def compute_observations(self):
    ...
    if self.cfg.terrain.measure_heights:
      heights = torch.clip(self.root_states[:, 2].unsqueeze(
          1) - 0.3 - self.measured_heights, -1, 1.)
      self.obs_buf = torch.cat([obs_buf, heights, priv_explicit, priv_latent,
                               self.obs_history_buf.view(self.num_envs, -1)], dim=-1)
    else:
      self.obs_buf = torch.cat([obs_buf, priv_explicit, priv_latent,
                               self.obs_history_buf.view(self.num_envs, -1)], dim=-1)
  ...
  def _post_physics_step_callback(self):
    ...
    if self.cfg.commands.heading_command:
      ...
      self.commands[:, 2] *= torch.abs(self.commands[:, 2]
  ...
  def _resample_commands(self, env_ids):
    ...
    if self.cfg.commands.heading_command:
      ...
      self.commands[env_ids,
                    2] *= torch.abs(self.commands[env_ids, 2]) > self.cfg.commands.ang_vel_clip

    # set small commands to zero
    self.commands[env_ids,
                  :2] *= torch.abs(self.commands[env_ids, 0:1]) > self.cfg.commands.lin_vel_clip
    ...
    if control_type=="P":
      if not self.cfg.domain_rand.randomize_motor:  # TODO add strength to gain directly
        torques = self.p_gains* \
            (actions_scaled + self.default_dof_pos_all - \
             self.dof_pos) - self.d_gains*self.dof_vel
      else:
        torques = self.motor_strength[0] * self.p_gains*(
            actions_scaled + self.default_dof_pos_all - self.dof_pos) - self.motor_strength[1] * self.d_gains*self.dof_vel
    ...
    if self.custom_origins:
      ...
      if self.cfg.env.randomize_start_pos:
        ...
        self.root_states[env_ids,
                         :2] += torch_rand_float(-0.3, 0.3, (len(env_ids), 2), device=self.device)
  ...
  def _update_terrain_curriculum(self, env_ids):
    ...
    self.env_class[env_ids] = self.terrain_class[self.terrain_levels[env_ids],
                                                 self.terrain_types[env_ids]]

    temp = self.terrain_goals[self.terrain_levels, self.terrain_types]
    last_col = temp[:, -1].unsqueeze(1)
    self.env_goals[:] = torch.cat((temp, last_col.repeat(
        1, self.cfg.env.num_future_goal_obs, 1)), dim=1)[:]
    self.cur_goals = self._gather_cur_goals()
    self.next_goals = self._gather_cur_goals(future=1)
  ...
  def _init_buffers(self):
    ...
    self.force_sensor_tensor = gymtorch.wrap_tensor(force_sensor_tensor).view(
        self.num_envs, 4, 6) # for feet only, see create_env()
    ...
    self._resample_commands(torch.arange(
  ...
  def _create_envs(self):
    ...
    for i in tqdm(range(self.num_envs)):
      ...
      self.mass_params_tensor[i, :] = torch.from_numpy(
          mass_params).to(self.device).to(torch.float)
    if self.cfg.domain_rand.randomize_friction:
      self.friction_coeffs_tensor = self.friction_coeffs.to(
          self.device).to(torch.float).squeeze(-1)
  ...
  def _get_env_origins(self):
    ...
    if self.cfg.terrain.mesh_type in ["heightfield", "trimesh"]:
      ...
      self.terrain_class = torch.from_numpy(
          self.terrain.terrain_type).to(self.device).to(torch.float)
      self.env_class[:] = self.terrain_class[self.terrain_levels, self.terrain_types]

      self.terrain_goals = torch.from_numpy(
          self.terrain.goals).to(self.device).to(torch.float)
      self.env_goals = torch.zeros(self.num_envs, self.cfg.terrain.num_goals + \
                                   self.cfg.env.num_future_goal_obs, 3, device=self.device, requires_grad=False)
  ...
  def _draw_goals(self):
    ...
    for i, goal in enumerate(goals):
      ...
      goal_z = self.height_samples[pts[0], pts[1]
                                   ].cpu().item() * self.terrain.cfg.vertical_scale
      pose = gymapi.Transform(gymapi.Vec3(goal[0], goal[1], goal_z), r=None)
      if i == self.cur_goal_idx[self.lookat_id].cpu().item():
        gymutil.draw_lines(sphere_geom_cur, self.gym, self.viewer,
                           self.envs[self.lookat_id], pose)
        if self.reached_goal_ids[self.lookat_id]:
          gymutil.draw_lines(sphere_geom_reached, self.gym,
                             self.viewer, self.envs[self.lookat_id], pose)
      else:
    ...
    if not self.cfg.depth.use_camera:
      ...
      for i in range(5):
        norm = torch.norm(self.target_pos_rel, dim=-1, keepdim=True)
        target_vec_norm = self.target_pos_rel / (norm + 1e-5)
        pose_arrow = pose_robot[:2] + 0.1* \
            (i+3) * target_vec_norm[self.lookat_id, :2].cpu().numpy()
        pose = gymapi.Transform(gymapi.Vec3(
            pose_arrow[0], pose_arrow[1], pose_robot[2]), r=None)
        gymutil.draw_lines(sphere_geom_arrow, self.gym, self.viewer,
                           self.envs[self.lookat_id], pose)
  ...
  def _draw_feet(self):
    if hasattr(self, 'feet_at_edge'):
      ...
      for i in range(4):
        pose = gymapi.Transform(gymapi.Vec3(
            feet_pos[self.lookat_id, i, 0], feet_pos[self.lookat_id, i, 1], feet_pos[self.lookat_id, i, 2]), r=None)
  ...
  def _init_height_points(self):
    ...
    for i in range(self.num_envs):
      offset = torch_rand_float(-self.cfg.terrain.measure_horizontal_noise,
                                self.cfg.terrain.measure_horizontal_noise, (self.num_height_points,2), device=self.device).squeeze()
      xy_noise = torch_rand_float(-self.cfg.terrain.measure_horizontal_noise, self.cfg.terrain.measure_horizontal_noise,
  ...
  def _reward_feet_edge(self):
    feet_pos_xy = ((self.rigid_body_states[:, self.feet_indices, :2] + self.terrain.cfg.border_size) / \
                   self.cfg.terrain.horizontal_scale).round().long()  # (num_envs, 4, 2)
...
# ----- final wrapper by luke ----- #

class RSLEnv():

  def __init__(self, configs=None):
    """
    Wrapper class which contains an RSL legged robot gym environment, specifically
    Go1 navigating on rough terrain. This is from the extreme-parkour code.
    https://github.com/chengxuxin/extreme-parkour

    This wrapper takes as input the configs from the hydra yaml file, and exposes
    neatly the reset, step, and seed functionality.

    The environment uses pytorch.
    """

    print(f"The root path for parkour is: '{LEGGED_GYM_ROOT_DIR}'")

    # create the task registry object which builds the environment
    ...
    # map the inputs of this function to their expected arg_parser
    rsl_args = get_args()
    env_name = rsl_args.task

    # specialised settings for Ash
    rsl_args.num_envs = 2048 # ash GPU cannot handle more
    rsl_args.headless = True # segfault if not, no GUI rendering available

    # get and save the env configs but also the training configs
    env_cfg, train_cfg = task_registry.get_cfgs(env_name)
    self.env_cfg, self.train_cfg = update_cfg_from_args(env_cfg, train_cfg, rsl_args)

    # the trainer needs the configs as a dictionary
    self.train_cfg_dict = class_to_dict(train_cfg)

    # now make the environment
    self.env, self.env_cfg = task_registry.make_env(name=env_name, args=rsl_args)

  def step(self, actions):
    """
    Step all the environments, given a tensor of actions with shape:
      (num_envs, num_actions_per_env)

    Returns (all vectorised for each environment):
      - observations
  ...
  def reset(self):
    """
    Reset all environments. Returns None.
    """
    return self.env.reset()

  def reset_idx(self, env_ids):
    """
    Reset the environment state of specific environments. Returns None.
    """
    self.env.reset_idx(env_ids)
    return None

  def seed(self, rngseed):
    """
    Seeding appears to be handled by the config file, so this function does nothing
    for now.
    """
    pass
